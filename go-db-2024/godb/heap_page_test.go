...
func TestHeapPageSerialization(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Error loading heap page from buffer. %v", err)
	}
...
func TestHeapPageBufferLen(t *testing.T) {
	...
	if buf.Len() != PageSize {
		t.Fatalf("HeapPage.toBuffer returns buffer of unexpected size %d, not %d;  NOTE:  This error may be OK, but many implementations that don't write full pages break.", buf.Len(), PageSize)
	}
}
